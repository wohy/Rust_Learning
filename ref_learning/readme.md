...
- 引用计数器
Arc 内部使用了 Atomic usize 引用计数，而非普通的 usize ，它是 usize 的原子类型，它使用了 cpu 的特殊指令来保证多线程下的安全
...
Cell 或 RefCell
- 所有权检查
检查发生在运行时，而非编译时，所以不符合规则，将直接产生 panic
...
但 ```Box::leak()``` 它创建的对象，从堆内存上泄漏出去，不受栈内存控制，是一个自由的、生命周期可以大到和 整个进程的生命周期一致（静态生命周期） 的对象。
